...
grep -q "volumes.data.mode=dynamic" <<<"$hlog"; check "no Retain class by default (no dynamic mode)" "1" "$?"
rm -f "${K8S_DIR}/values-user.yaml"

# --- case 8: neo4jStorageClass routes the data PVC to the Retain class ---
cat > "${K8S_DIR}/values-user.yaml" <<'EOF'
# Generated by bootstrap.sh
namespace: harness6-system
openaiApiUrl: 'https://api.example.com/v1'
...
graphitiGroupId: 'main'
semaphoreLimit: '5'
neo4jStorageClass: 'harness6-retain'
EOF
: > "${TMP}/kubectl.log"; : > "${TMP}/helm.log"
KUBECTL_LOG="${TMP}/kubectl.log" HELM_LOG="${TMP}/helm.log" \
  PATH="${FAKE_BIN}:${PATH}" bash "$DEPLOY" >/dev/null 2>&1; rc=$?
check "deploy.sh exits 0 with Retain class" "0" "$rc"
hlog="$(cat "${TMP}/helm.log")"
grep -q -e "--set volumes.data.mode=dynamic" <<<"$hlog"; check "Retain class switches to dynamic mode" "0" "$?"
grep -q "volumes.data.dynamic.storageClassName=harness6-retain" <<<"$hlog"; check "dynamic storageClassName from neo4jStorageClass" "0" "$?"
grep -q "volumes.data.dynamic.requests.storage=10Gi" <<<"$hlog"; check "dynamic PVC keeps 10Gi" "0" "$?"
rm -f "${K8S_DIR}/values-user.yaml"

# --- case 9: ingressClassName in values-user applies the graphiti Ingress ---
cat > "${K8S_DIR}/values-user.yaml" <<'EOF'
# Generated by bootstrap.sh
namespace: harness6-system
openaiApiUrl: 'https://api.example.com/v1'
...
KUBECTL_LOG="${TMP}/kubectl.log" HELM_LOG="${TMP}/helm.log" \
  PATH="${FAKE_BIN}:${PATH}" bash "$DEPLOY" >/dev/null 2>&1; rc=$?
check "deploy.sh exits 0 with ingressClassName" "0" "$rc"
klog="$(cat "${TMP}/kubectl.log")"
grep -q "graphiti-mcp-ingress-harness6-system.yaml" <<<"$klog"; check "ingressClassName applies graphiti Ingress" "0" "$?"
rm -f "${K8S_DIR}/values-user.yaml"
